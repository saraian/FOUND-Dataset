...
# Keep datasets out of the source repository.  A checkout can either place them
# under ``habitat/`` next to this module, or point to a shared installation with
# HABITAT_DATASETS_DIR.  The old absolute /root/exchange path deliberately is
# not a fallback: it made every new checkout machine-specific.
_HABITAT_DATASETS_DIR = os.path.abspath(os.path.expanduser(os.environ.get(
    "HABITAT_DATASETS_DIR", os.path.join(_PACKAGE_DIR, "habitat")
)))


def _habitat_dataset_path(*parts):
    return os.path.join(_HABITAT_DATASETS_DIR, *parts)
...
_DEFAULTS = {
    ...
    "habitat": {  # HM3D (Matterport) scenes; paths derive from HABITAT_DATASETS_DIR
        "dataset_root": _HABITAT_DATASETS_DIR,
        "scene": _habitat_dataset_path("hm3d-val-habitat-v0.2", "00801-HaxA7YrQdEC", "HaxA7YrQdEC.basis.glb"),
        "scene_dataset": _habitat_dataset_path("hm3d-val-habitat-v0.2", "hm3d_annotated_basis.scene_dataset_config.json"),
        "nav_scene": _habitat_dataset_path("hm3d-val-habitat-v0.2", "00802-wcojb4TFT35", "wcojb4TFT35.basis.glb"),
        "nav_scene_dataset": _habitat_dataset_path("hm3d-val-semantic-configs-v0.2", "hm3d_annotated_basis.scene_dataset_config.json"),
        "nav_navmesh": _habitat_dataset_path("hm3d-val-habitat-v0.2", "00802-wcojb4TFT35", "wcojb4TFT35.basis.navmesh"),
        "width": 640,
        "height": 480,
...
def habitat_value(key):
    """Return a Habitat setting, with per-run environment overrides.

    ``HABITAT_DATASETS_DIR`` is consumed while building the portable defaults;
    the more specific variables retain the command-line workflow used by the
    script runner.
    """
    env_key = {
        "scene": "HABITAT_SCENE",
